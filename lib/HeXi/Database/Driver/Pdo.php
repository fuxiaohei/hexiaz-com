...
class PdoDriver extends AbstractDriver {
    ...
    public function __construct($name = 'default') {
        if (!extension_loaded('pdo')) {
            HeXi::exception('不支持PDO数据库连接', 'database');
        }
        ...
        switch (strtolower($this->config['type'])) {
            ...
            case 'mysql':
                ...
                break;
            default:
                HeXi::exception('PDO is not supported your database "' . $this->config['type'] . '"', 'database');
                break;
        }
    ...
    private function newSqlite() {
        if (!extension_loaded('pdo_sqlite')) {
            HeXi::exception('PDO is not supported for SQLite', 'databas');
        }
        if (!$this->config['file']) {
            HeXi::exception('Database "' . $this->named . '" Configuration is wrong !', 'database');
        }
    ...
    private function newMysql() {
        if (!extension_loaded('pdo_mysql')) {
            HeXi::exception('PDO is not supported for MySQL', 'database');
        }
        ...
        try {
            ...
            HeXi::exception($exc->getMessage(), 'database');
        }
        return $pdo;
    }
    ...
    public function error() {
        if (!$this->pdo) {
            HeXi::exception('Database Unknown Error', 'database');
        }
        $error = $this->pdo->errorInfo();
        #释放连接
        $this->pdo  = null;
        $this->stmt = null;
        HeXi::exception('Database Error - ' . $error[0] . '; ' . $error[2], 'database');
    }
